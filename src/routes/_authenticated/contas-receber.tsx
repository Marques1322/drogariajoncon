...
export const Route = createFileRoute("/_authenticated/contas-receber")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Parcelas de clientes, vencimentos, pagamentos parciais e totais.",
      },
    ],
  }),
  ...
  const listQ = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("contas_receber")
        .select(
          "id, descricao, valor, data_emissao, data_vencimento, data_recebimento, status, forma_pagamento, cliente:clientes(nome, limite_credito)",
        )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Parcelas de clientes, pagamentos totais ou parciais.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">Total exibido</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{brl(totais.total)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">Pendente</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{brl(totais.pendente)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">Atrasado</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold text-destructive">{brl(totais.atrasado)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">Recebido</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold text-emerald-600">{brl(totais.recebido)}</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <TableBody>
                    {listQ.isLoading ? (
                      ...
                      listQ.data!.map((r: any) => {
                        ...
                        const alertar =
                          r.status !== "pago" && r.status !== "cancelado" && dias <= 7;
                        return (
                          <TableRow key={r.id}>
      ...
      <ReceberDialog
        id={receberId}
        onOpenChange={() => setReceberId(null)}
        conta={(listQ.data ?? []).find((r: any) => r.id === receberId)}
        onDone={() => {
          setReceberId(null);
...
function ReceberDialog({
  ...
  useEffect(() => {
    if (conta) setValor(String(conta.valor));
  }, [conta]);
    ...
    <Dialog open={!!id} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Informe o valor recebido. Pagamentos parciais mantêm o saldo em aberto automaticamente.
          </DialogDescription>
        </DialogHeader>
        {conta && (
          <div className="space-y-4">
            <div className="rounded-md bg-muted/40 p-3 text-sm">
              ...
              <p className="text-xs text-muted-foreground">
                Cliente: {conta.cliente?.nome} • Saldo: {brl(Number(conta.valor))} • Venc.:{" "}
