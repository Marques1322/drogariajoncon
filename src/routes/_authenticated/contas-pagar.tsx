  ...
  const listQ = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("contas_pagar")
        .select(
          "id, descricao, valor, data_emissao, data_vencimento, data_pagamento, status, forma_pagamento, observacoes, fornecedor:fornecedores(razao_social), compra:compras(numero_nota)",
        )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Duplicatas vinculadas a compras, vencimentos e pagamentos.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xl font-semibold text-emerald-600">{brl(totais.pago)}</p>
          </CardContent>
        </Card>
      </div>
                        ...
                        return (
                          <TableRow key={r.id}>
                            <TableCell className="font-medium">
                              {r.descricao}
                              {r.compra?.numero_nota && (
                                <span className="text-xs text-muted-foreground ml-2">
                                  Nota {r.compra.numero_nota}
                                </span>
                              )}
                            </TableCell>
                            ...
                            <TableCell>
                              {r.data_pagamento
                                ? format(new Date(r.data_pagamento), "dd/MM/yyyy", { locale: ptBR })
                                : "—"}
                            </TableCell>
                            <TableCell>
                              <Badge
      ...
      <PagarDialog
        id={pagarId}
        onOpenChange={() => setPagarId(null)}
        conta={(listQ.data ?? []).find((r: any) => r.id === pagarId)}
        onDone={() => {
          setPagarId(null);
    ...
    <Dialog open={!!id} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Informe o valor pago. Se for menor que o saldo, o restante permanece pendente como nova
            parcela.
          </DialogDescription>
        </DialogHeader>
        {conta && (
          <div className="space-y-4">
            <div className="rounded-md bg-muted/40 p-3 text-sm">
              ...
              <p className="text-xs text-muted-foreground">
                Saldo: {brl(Number(conta.valor))} • Venc.:{" "}
